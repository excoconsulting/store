<?php
namespace WalkerEdgeNamespace\Modules\Shortcodes\TeamCarousel;
...
class TeamCarousel implements ShortcodeInterface {
	...
	public function vcMap()	{
		...
		for ($x = 1; $x<6; $x++) {
			$teamIconCollections = walker_edge_icon_collections()->getCollectionsWithSocialIcons();
			...
			$team_social_icons_array[] = array(
				...
				'dependency' => array('element' => 'team_social_icon_pack', 'value' => walker_edge_icon_collections()->getIconCollectionsKeys())
			);
		...
		vc_map( array(
			'name' => esc_html__('Edge Team Carousel Item', 'walker'),
			'base' => $this->getBase(),
			...
			'params' => array_merge(
				array(
					...
					array(
						...
						'admin_label' => true,
						'value' => array_merge(array('' => ''),walker_edge_icon_collections()->getIconCollectionsVCExclude('linea_icons')),
						'save_always' => true
					)
				),
	...
	public function render($atts, $content = null) {
		...
		$html = walker_edge_get_shortcode_module_template_part('templates/team-carousel', 'team-carousels', '', $params);
	...
	private function getTeamSocialIcons($params) {
		...
		if ($team_social_icon_pack !== '') {
			...
			$icon_pack = walker_edge_icon_collections()->getIconCollection($team_social_icon_pack);
			...
			for ( $i = 1; $i <= $number_of_social_icons; $i++ ) {
				...
				if ($team_social_icon !== '') {
					...
					$social_icons[] = walker_edge_execute_shortcode('edgtf_icon', $team_icon_params);
				}
			}
		}
