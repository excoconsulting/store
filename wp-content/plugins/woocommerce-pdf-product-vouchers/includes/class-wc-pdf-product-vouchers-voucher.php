...
class WC_PDF_Product_Vouchers_Voucher {
	...
	public function generate_voucher_pdf( $order_id, $force = false ) {
		...
		foreach ( WC_PDF_Product_Vouchers_Order::get_vouchers( $order ) as $voucher ) {
			...
			if ( ! is_writable( $voucher_pdf_path ) ) {
				/* translators: Placeholders: %1$s - voucher number, %2$s - voucher pdf path */
				$order->add_order_note( sprintf( __( 'PDF Product Vouchers file permission error: unable to generate voucher %1$s in %2$s.  Please fix the directory permissions and re-generate the voucher.', 'woocommerce-pdf-product-vouchers' ), $voucher->get_voucher_number(), $voucher_pdf_path ) );
				continue;
			}
			...
			try {
				...
				$order->add_order_note( sprintf( __( 'PDF Product Vouchers: unable to generate voucher %1$s due to: %2$s', 'woocommerce-pdf-product-vouchers' ), $voucher->get_voucher_number(), $exception->getMessage() ) );
			}
		}
	}
	...
	public function add_voucher_number_to_email_order_item_meta( $item_id, $item, $order ) {
		...
		if ( $item['product_id'] > 0 && isset( $item['voucher_id'] ) && $item['voucher_id'] ) {
			...
			echo '<br/><small>' . __( 'Voucher Number: ', 'woocommerce-pdf-product-vouchers' ) . $voucher->get_voucher_number() . '</small>';
		}
	}
