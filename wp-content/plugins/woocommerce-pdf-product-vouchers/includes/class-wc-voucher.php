...
class WC_Voucher {
	...
	public function get_voucher_number() {
		...
		if ( $this->order_id ) {
			$voucher_number = ltrim( $this->get_order()->get_order_number(), _x( '#', 'hash before order number', 'woocommerce-pdf-product-vouchers' ) ) . '-' . $voucher_number;
		}
	...
	private function wc_price( $price ) {
		...
		if ( 0 == $price ) {
			return __( 'Free!', 'woocommerce-pdf-product-vouchers' );
		}
		...
		$decimal_sep     = wp_specialchars_decode( stripslashes( wc_get_price_decimal_separator() ), ENT_QUOTES );
		$thousands_sep   = wp_specialchars_decode( stripslashes( wc_get_price_thousand_separator() ), ENT_QUOTES );
		...
		foreach ( $this->voucher_fields as $field ) {
			if ( 'user_input' == $field['type'] ) {
				foreach ( $this->item as $meta_name => $meta_value ) {
					if ( __( $field['label'], 'woocommerce-pdf-product-vouchers' ) == $meta_name ) {
	...
	public function generate_pdf( $path = '' ) {
		...
		if ( ! $image ) {
			throw new Exception( __( "Voucher image not found", 'woocommerce-pdf-product-vouchers' ) );
		}

		// make sure the file exists and is readable
		if ( ! is_readable( $upload_dir['basedir'] . '/' . $image['file'] ) ) {
			/* translators: Placeholders: %s - image path */
			throw new Exception( sprintf( __( 'Voucher image file missing or not readable: %s', 'woocommerce-pdf-product-vouchers' ), $upload_dir['basedir'] . '/' . $image['file'] ) );
		}
	...
	public function get_item_meta_value( $meta_name ) {
		...
		foreach ( $this->item['item_meta'] as $name => $value ) {
			if ( __( $meta_name, 'woocommerce-pdf-product-vouchers' ) == $name ) {
				return $value[0];
			}
		}
