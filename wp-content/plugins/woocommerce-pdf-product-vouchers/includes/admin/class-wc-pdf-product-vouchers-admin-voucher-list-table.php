...
class WC_PDF_Product_Vouchers_Admin_Voucher_List_Table {
	...
	public function edit_voucher_columns( $columns ){
		...
		$columns['name']           = __( 'Name', 'woocommerce-pdf-product-vouchers' );
		...
		switch ( $column ) {
			...
			case 'name':
				...
				if ( 'draft' == $post->post_status ) {
					echo " - <span class='post-state'>" . __( 'Draft', 'woocommerce-pdf-product-vouchers' ) . '</span>';
				}
				...
				if ( current_user_can( $post_type_object->cap->delete_post, $post->ID ) ) {
					if ( 'trash' == $post->post_status )
						$actions['untrash'] = "<a title='" . esc_attr( __( 'Restore this item from the Trash', 'woocommerce-pdf-product-vouchers' ) ) . "' href='" . wp_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-' . $post->post_type . '_' . $post->ID ) . "'>" . __( 'Restore', 'woocommerce-pdf-product-vouchers' ) . "</a>";
					elseif ( EMPTY_TRASH_DAYS )
						$actions['trash'] = "<a class='submitdelete' title='" . esc_attr( __( 'Move this item to the Trash', 'woocommerce-pdf-product-vouchers' ) ) . "' href='" . get_delete_post_link( $post->ID ) . "'>" . __( 'Trash', 'woocommerce-pdf-product-vouchers' ) . "</a>";
					if ( 'trash' == $post->post_status || ! EMPTY_TRASH_DAYS )
						$actions['delete'] = "<a class='submitdelete' title='" . esc_attr( __( 'Delete this item permanently', 'woocommerce-pdf-product-vouchers' ) ) . "' href='" . get_delete_post_link( $post->ID, '', true ) . "'>" . __( 'Delete Permanently', 'woocommerce-pdf-product-vouchers' ) . "</a>";
				}
