...
class WC_PDF_Product_Vouchers_Admin_Orders {
	...
	public function maybe_complete_order( $post_id, $post ) {
		...
		wc_pdf_product_vouchers()->get_voucher_handler_instance()->generate_voucher_pdf( $post_id );
	...
	public function download_permissions_title( $title, $product_id, $order_id, $order_key, $download_id ) {
		...
		if ( $voucher ) {
			$title .= ' (' . sprintf( __( 'Voucher %s', 'woocommerce-pdf-product-vouchers' ), $voucher->get_voucher_number() ) . ')';
		}
	...
	public function add_vouchers_meta_box() {
		...
		add_meta_box(
			...
			__( 'Vouchers', 'woocommerce-pdf-product-vouchers' ),
	...
	public function vouchers_meta_box( $post ) {
		...
		<div class="woocommerce_order_vouchers_wrapper">
			<table cellpadding="0" cellspacing="0" class="woocommerce_order_vouchers">
				<thead>
					<tr>
						<th class="thumb" width="1%"><?php _e( 'Voucher', 'woocommerce-pdf-product-vouchers' ); ?></th>
						<th class="voucher_number"><?php _e( 'Number', 'woocommerce-pdf-product-vouchers' ); ?></th>
						<th class="sku"><?php _e( 'SKU', 'woocommerce-pdf-product-vouchers' ); ?></th>
						<th class="data"><?php _e( 'Data', 'woocommerce-pdf-product-vouchers' ); ?></th>
						<th class="expires"><?php _e( 'Expires', 'woocommerce-pdf-product-vouchers' ); ?></th>
						<th class="qty"><?php _e( 'Qty', 'woocommerce-pdf-product-vouchers' ); ?></th>
						<th class="redeem" style="white-space:nowrap;">
							<?php _e( 'Mark Redeemed', 'woocommerce-pdf-product-vouchers' ); ?>&nbsp;
							<?php echo SV_WC_Plugin_Compatibility::wc_help_tip( __( 'Mark the dates that any vouchers are redeemed.  Marking all vouchers redeemed will complete the order.', 'woocommerce-pdf-product-vouchers' ) ); ?>
						</th>
					</tr>
				</thead>
						...
						<tr class="item" rel="<?php echo $item_id; ?>">
							<td class="thumb">
								<a href="<?php echo esc_url( admin_url( 'post.php?post='. $voucher->id . '&action=edit' ) ); ?>" class="tips" data-tip="<?php
									echo '<strong>' . __( 'Voucher ID:', 'woocommerce-pdf-product-vouchers' ).'</strong> ' . $voucher->id;
								?>"><?php echo $voucher->get_image(); ?></a>
								<?php if ( $voucher->file_exists( WC_PDF_Product_Vouchers::get_uploads_path() ) ) : ?>
									<a href="<?php echo esc_url( add_query_arg( array( 'action' => 'download', 'product_id' => $item['product_id'], 'item_id' => $item_id, 'voucher_id' => $item['voucher_id'] ) ) ); ?>"><?php esc_html_e( 'Download', 'woocommerce-pdf-product-vouchers' ); ?></a>
								<?php endif; ?>
							</td>
		...
		<p class="buttons buttons-alt">
			<button type="button" class="button redeem_all_vouchers"><?php _e( 'Redeem All &uarr;', 'woocommerce-pdf-product-vouchers' ); ?></button>
		</p>
		<div class="clear"></div>
		<?php
