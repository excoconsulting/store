...
class WC_PDF_Product_Vouchers_Admin_Products {
	...
	public function product_options_downloads() {
		...
		foreach ( wc_pdf_product_vouchers()->get_voucher_handler_instance()->get_vouchers() as $voucher ) {
			$options[ $voucher->ID ] = $voucher->post_title;
		}

		woocommerce_wp_select( array( 'id' => '_voucher_id', 'label' => __( 'Voucher', 'woocommerce-pdf-product-vouchers' ), 'options' => $options, 'desc_tip' => true, 'description' => __( 'Select a voucher template rather than supplying a File Path to make this into a voucher product.', 'woocommerce-pdf-product-vouchers' ) )  );
		...
		?>
			<div class="show_if_variation_downloadable" style="display:none;">
				<p class="form-row form-row-first">
					<label>
						<?php
							_e( 'Voucher:', 'woocommerce-pdf-product-vouchers' );
							echo SV_WC_Plugin_Compatibility::wc_help_tip( __( 'Select a voucher rather than providing a file path', 'woocommerce-pdf-product-vouchers' ) );
						?>
					</label>
					<select class="variable_voucher" name="variable_voucher_id[<?php echo $loop; ?>]">
						<?php
