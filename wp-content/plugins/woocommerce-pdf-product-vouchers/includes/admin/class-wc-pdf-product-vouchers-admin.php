...
class WC_PDF_Product_Vouchers_Admin {
	...
	public function __construct( $plugin ) {
		...
		$this->list_table = $this->plugin->load_class( '/includes/admin/class-wc-pdf-product-vouchers-admin-voucher-list-table.php', 'WC_PDF_Product_Vouchers_Admin_Voucher_List_Table' );

		$this->admin_vouchers = $this->plugin->load_class( '/includes/admin/class-wc-pdf-product-vouchers-admin-vouchers.php', 'WC_PDF_Product_Vouchers_Admin_Vouchers' );
	...
	public function product_updated_messages( $messages ) {
		...
		$messages['wc_voucher'] = array(
			...
			5 => isset( $_GET['revision'] ) ? sprintf( __( 'Voucher restored to revision from %s', 'woocommerce-pdf-product-vouchers' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6 => __( 'Voucher updated.', 'woocommerce-pdf-product-vouchers' ),
			7 => __( 'Voucher saved.', 'woocommerce-pdf-product-vouchers' ),
			8 => __( 'Voucher submitted.', 'woocommerce-pdf-product-vouchers' ),
			9 => sprintf( __( 'Voucher scheduled for: <strong>%1$s</strong>.', 'woocommerce-pdf-product-vouchers' ),
			  date_i18n( __( 'M j, Y @ G:i', 'woocommerce-pdf-product-vouchers' ), strtotime( $post->post_date ) ) ),
		...
		if ( in_array( $screen->id, array( 'shop_order', 'wc_voucher' ) ) ) {
			...
			if ( 'wc_voucher' == $screen->id ) {
				...
				$woocommerce_vouchers_params = array(
					'done_label'           => __( 'Done', 'woocommerce-pdf-product-vouchers' ),
					'set_position_label'   => __( 'Set Position', 'woocommerce-pdf-product-vouchers' ),
					'post_id'              => $post->ID,
	...
	public function help_tab() {
		...
		$screen->add_help_tab( array(
			...
			'title'   => __( 'Overview', 'woocommerce-pdf-product-vouchers' ),
			'content' => '<p>' . __( 'The WooCommerce PDF Product Vouchers plugin allows you to create and configure customizable vouchers which can be attached to simple/variable downloadable products and purchased by your customers.  You can give your customers the ability to set a recipient name/message and purchase these vouchers as a gift, and allow them to choose from among a set of voucher images.  Once the purchase is made the customer will have access to a custom-generated PDF voucher in the same manner as a standard downloadable product.', 'woocommerce-pdf-product-vouchers' ) . '</p>',
		) );

		$screen->add_help_tab( array(
			'id'       => 'wc_vouchers_voucher_help_tab',
			'title'    => __( 'Editing a Voucher', 'woocommerce-pdf-product-vouchers' ),
			'callback' => array( $this, 'help_tab_content' ),
		) );

		$screen->add_help_tab( array(
			'id'      => 'wc_vouchers_list_help_tab',
			'title'   => __( 'Vouchers List', 'woocommerce-pdf-product-vouchers' ),
			'content' => '<p>' . __( 'From the list view you can review all your voucher templates, quickly see the name, primary default image and optional expiry days, and trash a voucher template.', 'woocommerce-pdf-product-vouchers' ) . '</p>'
		) );

		$screen->add_help_tab( array(
			'id'       => 'wc_vouchers_how_to_help_tab',
			'title'    => __( 'How To', 'woocommerce-pdf-product-vouchers' ),
			'callback' => array( $this, 'how_to_help_tab_content' ),
		) );

		$screen->set_help_sidebar(
			'<p><strong>' . __( 'For more information:', 'woocommerce-pdf-product-vouchers' ) . '</strong></p>' .
			'<p><a href="http://docs.woothemes.com/document/pdf-product-vouchers/" target="_blank">' . __( 'Vouchers Docs', 'woocommerce-pdf-product-vouchers' ) . '</a></p>'
		);
	}
	...
	public function help_tab_content() {
		?>
		<p><strong><?php _e( 'Voucher Name', 'woocommerce-pdf-product-vouchers' ) ?></strong> - <?php _e( 'All voucher templates must be given a name.  This will be used to identify the voucher within the admin; from the frontend the voucher will be identified to the customer by a unique voucher number.', 'woocommerce-pdf-product-vouchers' ) ?></p>
		<p><strong><?php _e( 'Primary Voucher Image', 'woocommerce-pdf-product-vouchers' ) ?></strong> - <?php _e( 'This is the main image for your voucher, and will be used to configure the layout of the various text fields defined in the Voucher Data panel.', 'woocommerce-pdf-product-vouchers' ) ?></p>
		<p><strong><?php _e( 'Voucher Data', 'woocommerce-pdf-product-vouchers' ) ?></strong> - <?php _e( 'These configuration options allow you to specify exactly where various text fields will be displayed on your voucher, as well as the font used.  For instance, if you want the product name displayed on your voucher, click the "Set Position" button next to "Product Name Position".  Then select the area of the Voucher Image where you want the product name to be displayed.', 'woocommerce-pdf-product-vouchers' ) ?></p>
		<p><?php _e( 'You can define a default font, size, style and color to be used for the voucher text fields.  For each individual text field, you can override these defaults by setting a specific font/style, size or color.  Note that the default font style (Italic/Bold) will only be used if a font is not selected at the field level.', 'woocommerce-pdf-product-vouchers' ) ?></p>
		<p><strong><?php _e( 'Alternative Images', 'woocommerce-pdf-product-vouchers' ) ?></strong> - <?php _e( 'You can add alternative voucher images so your customers can choose from multiple backgrounds when purchasing a voucher.  Just make sure all the images have the same layout so the voucher text fields are put in the correct position.', 'woocommerce-pdf-product-vouchers' ) ?></p>
		<p><strong><?php _e( 'Additional Image', 'woocommerce-pdf-product-vouchers' ) ?></strong> - <?php _e( 'You can add a second page to the voucher with this option, containing for instance voucher instructions or policies.  As with the alternative images, ensure that this additional image has the same dimensions as the primary voucher image.', 'woocommerce-pdf-product-vouchers' ) ?></p>
		<p><strong><?php _e( 'Previewing', 'woocommerce-pdf-product-vouchers' ) ?></strong> - <?php _e( 'You must update the voucher to see any changes in the voucher Preview.', 'woocommerce-pdf-product-vouchers' ) ?></p>
		<?php
	}
	...
	public function how_to_help_tab_content() {
		?>
		<p><strong><?php _e( 'How to Create Your First Voucher Product', 'woocommerce-pdf-product-vouchers' ) ?></strong></p>
		<ol>
			<li><?php _e( 'First go to WooCommerce &gt; Vouchers and click "Add Voucher" to add a voucher template', 'woocommerce-pdf-product-vouchers' ); ?></li>
			<li><?php _e( 'Set a Voucher Name, and Primary Voucher Image.  Optionally configure and add some Voucher Data fields (see the "Editing a Voucher" section for more details)', 'woocommerce-pdf-product-vouchers' ); ?></li>
			<li><?php _e( 'Next click "Publish" to save your voucher template.  You can also optionally "Preview" the voucher to check your work and field layout.', 'woocommerce-pdf-product-vouchers' ); ?></li>
			<li><?php _e( 'Next go to WooCommerce &gt; Products and either create a new product or edit an existing one, being sure to make it either Simple or Variable, and checking the "Downloadable" option (probably also checking the "Virtual" option, unless you plan on mailing hard copies of the product Vouchers).', 'woocommerce-pdf-product-vouchers' ); ?></li>
			<li><?php _e( 'With the "Downloadable" option checked on a Simple type product you should see a field named "Voucher" in the General Product Data tab with a select box containing your newly created voucher template.  With a "Variable" type product the field will appear in the Variations area.  Select your voucher and save the product.', 'woocommerce-pdf-product-vouchers' ); ?></li>
			<li><?php _e( 'Your product voucher is now available for purchase!  Run a test transaction from the frontend, you should see the voucher primary image(s) displayed on the product page, along with the optional Recipient Name/Messag fields if you added them.  The voucher PDF will be available via downloadable link the same as any standard downloadable produt.', 'woocommerce-pdf-product-vouchers' ); ?></li>
		</ol>
		<?php
	}
