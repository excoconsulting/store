		...
		foreach ( $downloads as $download ) {
			...
			if ( false !== strpos( $download['download_id'], 'wc_vouchers_' ) && WC_PDF_Product_Vouchers_Product::has_voucher( $product ) ) {
				...
				if ( $voucher && $voucher->file_exists( WC_PDF_Product_Vouchers::get_uploads_path() ) ) {
					...
					/* translators: Placeholders: %1$s - voucher number, %2$s - order date */
					$download['download_name'] .= ' (' . sprintf( _x( 'Voucher %1$s %2$s', 'Voucher number and date', 'woocommerce-pdf-product-vouchers' ), $voucher->get_voucher_number(), date_i18n( get_option( 'date_format' ), strtotime( $order->order_date ) ) ) . ')';
