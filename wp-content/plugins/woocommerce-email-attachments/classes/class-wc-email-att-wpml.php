...
class WC_Email_Att_WPML 
{
	...
	{		
		...
		if( ! version_compare( self::MIN_WPML_VERSION, ICL_SITEPRESS_VERSION, '<='  ))  { return;  }
		$this->version_conflict = false;
		
		if( ! function_exists( 'wpml_add_translatable_content' ) )
		{
			if( ! file_exists( WP_PLUGIN_DIR . '/sitepress-multilingual-cms/inc/wpml-api.php' ) )   {  return;  }
			include_once( WP_PLUGIN_DIR . '/sitepress-multilingual-cms/inc/wpml-api.php' );
		}
		
			//	check, that all functions exist, otherwise keep object inactive
		if( ! function_exists( 'wpml_add_translatable_content' ) ) {   return;   }
