				...
				if ( $add_on->has_options() ) {
					...
					<select name="<?php echo esc_attr( $add_on->get_key() ); ?>" id="<?php echo esc_attr( $add_on->get_key() ); ?>" class="wc-enhanced-select" data-placeholder="<?php echo esc_attr( __( "Show all ", 'woocommerce-checkout-add-ons' ) . $add_on->name ); ?>" data-allow_clear="true" style="min-width:200px;">
						...
						foreach ( $add_on->get_options() as $option ) :
							...
							echo '<option value="' . esc_attr( $option['value'] ) . '" ' . ( isset( $_GET[ $add_on->get_key() ] ) ? selected( $option['value'], $_GET[ $add_on->get_key() ] ) : '' ) . '>' . esc_html__( $option['label'], 'woocommerce-checkout-add-ons' ) . '</option>';
						endforeach;
						?>
					</select>
					<?php
