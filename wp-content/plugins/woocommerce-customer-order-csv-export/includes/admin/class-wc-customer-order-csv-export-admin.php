...
class WC_Customer_Order_CSV_Export_Admin {
	...
	/** @var \WC_Customer_Order_CSV_Export_Admin_Column_Mapper instance */
	...
	public function render_submenu_pages() {
		...
		$this->tabs = array(
			...
			'columns'     => __( 'Columns', 'woocommerce-customer-order-csv-export' ),
		);
		...
		// save column mapping
		if ( ! empty( $_POST ) && 'columns' === $current_tab ) {

			$this->get_column_mapper_instance()->save();
		}
		...
		} elseif ( 'columns' === $current_tab ) {

			$this->get_column_mapper_instance()->output();
	...
	public function add_order_action( $order ) {
		...
		if ( ! get_post_meta( $order->id, '_wc_customer_order_csv_export_is_exported', true ) ) {
			...
			printf( '<a class="button tips %1$s" href="#" data-tip="%2$s">%s</a>', $action, $name, $name );
		}
	 ...
	 *
	 * @since 4.0.0
	 * @return \WC_Customer_Order_CSV_Export_Admin_Column_Mapper instance
	 */
	public function get_column_mapper_instance() {

		if ( ! isset( $this->column_mapper ) ) {

			$this->column_mapper = wc_customer_order_csv_export()->load_class( '/includes/admin/class-wc-customer-order-csv-export-admin-column-mapper.php', 'WC_Customer_Order_CSV_Export_Admin_Column_Mapper' );
		}
