		...
		<p class="form-field">
			<label>
				...
				<?php
					...
					if ( $field_type == 'global' ) {
						$tiptip = __( 'Defaults to:<br/>&quot;Unfortunately your order cannot be shipped {to_excluded_destination}. To complete your order, please select an alternative shipping country / state.&quot;<br/>When conditions are defined, the default message will be amended with resolution instructions.', 'woocommerce-conditional-shipping-and-payments' );
					} else {
						$tiptip = __( 'Defaults to:<br/>&quot;Unfortunately your order cannot be shipped {to_excluded_destination}. To complete your order, please select an alternative shipping country / state, or remove {product} from your cart.&quot;<br/>When conditions are defined, the default message will be amended with resolution instructions.', 'woocommerce-conditional-shipping-and-payments' );
					}
				?>
			</label>
			...
			<?php
				...
				if ( $field_type == 'global' ) {
					...
					$tip = __( 'Define a custom checkout error message to show when selecting an excluded shipping destination. You may include <code>{product}</code> and <code>{to_excluded_destination}</code> and have them substituted by the actual product title and country / state.', 'woocommerce-conditional-shipping-and-payments' );
				}
						...
						if ( ! empty( $restriction[ 'countries' ] ) && $this->check_conditions_apply( $restriction, array( 'cart_item_data' => $cart_item_data ) ) ) {
							...
							if ( in_array( $shipping_country, $restricted_countries ) ) {
								...
								if ( empty( $restriction[ 'states' ][ $shipping_country ] ) ) {
									...
									$to_destination             = sprintf( __( '%1$s %2$s', 'woocommerce-conditional-shipping-and-payments', 'to country destination' ), WC()->countries->shipping_to_prefix(), $destination );
									...
									$to_destination             = sprintf( __( 'to %s', 'woocommerce-conditional-shipping-and-payments', 'to state destination' ), $destination );
						...
						if ( $restriction_exists ) {
							...
							if ( ! empty( $restriction[ 'message' ] ) ) {
								...
								if ( $conditions_resolution ) {
									$resolution = sprintf( __( 'To have your order shipped %1$s, please %2$s. Otherwise, select an alternative shipping %3$s, or remove &quot;%4$s&quot; from your cart.', 'woocommerce-conditional-shipping-and-payments' ), $to_destination, $conditions_resolution, $destination_type, $product->get_title() );
								} else {
