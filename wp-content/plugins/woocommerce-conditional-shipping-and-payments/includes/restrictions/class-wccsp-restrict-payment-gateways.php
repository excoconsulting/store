...
class WC_CSP_Restrict_Payment_Gateways extends WC_CSP_Restriction implements WC_CSP_Checkout_Restriction {
	...
	public function get_admin_fields_html( $index, $options = array() ) {
		...
		<p class="form-field">
			...
			<?php echo WC_CSP_Core_Compatibility::wc_help_tip( __( 'By default, excluded payment gateways are removed from the list of gateways available during checkout. Select this option if you prefer to show excluded gateways in the checkout options and trigger a restriction notice when customers attempt to complete an order using an excluded gateway.', 'woocommerce-conditional-shipping-and-payments' ) ); ?>
		</p>
		<?php
	}
				...
				if ( ! empty( $cart_contents ) ) {
					...
					if ( ! empty( $restriction[ 'gateways' ] ) && $this->check_conditions_apply( $restriction ) ) {
						...
						if ( in_array( $chosen_gateway, $restricted_gateways ) ) {
							...
							if ( $conditions_resolution ) {
								$resolution = sprintf( __( 'To complete your order via &quot;%1$s&quot;, please %2$s. Otherwise, select an alternative payment method.', 'woocommerce-conditional-shipping-and-payments' ), $available_gateways[ $chosen_gateway ]->get_title(), $conditions_resolution );
							} else {
