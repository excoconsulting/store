...
class WC_Coupon_Campaigns {
	...
	public function __construct( $file ) {
		...
		if( is_admin() ) {
			...
			// Add coupon report menu items
			// add_filter( 'woocommerce_admin_reports', array( $this, 'report_menu_items' ) );
		}
	...
	/**
	 * Add campaign reports to report tabs
	 * @param  arr $charts Existing reports
	 * @return arr         Updated reports
	 */
	public function report_menu_items( $reports ) {

		$reports['coupons'] = array(
			'title'   => __( 'Coupons', 'wc_coupon_campaigns' ),
			'reports' => array(
				'campaigns' => array(
					'title'       => __( 'Campaigns', 'wc_coupon_campaigns' ),
					'description' => '',
					'hide_title'  => true,
					'callback'    => 'woocommerce_coupons_campigns_report'
				)
			)
		);
	...
	public function display_usage_notes( $post ) {
		...
		if( $usage_count > 0 ) {
			...
			$data .= '<br/>' . __( 'Total discount:', 'wc_coupon_campaigns' ) . ' ' . woocommerce_price( $total_discount ) . '<br/>
						' . __( 'Total revenue:', 'wc_coupon_campaigns' ) . ' ' . woocommerce_price( $total_revenue );
		}
	...
	private function save_order_data( $coupon, $order_id ) {
		...
		$campaigns = wp_get_post_terms( $coupon->id, $this->tax );
	...
	private function save_coupon_data( $coupon, $order_id, $order_total, $discount, $user, $time ) {
		...
		// Track customers who used this coupon
		$users = get_post_meta( $coupon->id, '_coupon_customers', true );
		$users[ $order_id ] = $user->ID;
		update_post_meta( $coupon->id, '_coupon_customers', $users );

		// Track orders in which this coupon was used
		$orders = get_post_meta( $coupon->id, '_coupon_orders', true );
		$orders[ $order_id ] = $order_id;
		update_post_meta( $coupon->id, '_coupon_orders', $orders );

		// Track all discount amounts given by this coupon
		$discounts = get_post_meta( $coupon->id, '_coupon_discounts', true );
		$discounts[ $order_id ] = $discount;
		update_post_meta( $coupon->id, '_coupon_discounts', $discounts );

		// Track all revenue amounts generated by this coupon
		$revenue = get_post_meta( $coupon->id, '_coupon_revenue', true );
		$revenue[ $order_id ] = $order_total;
		update_post_meta( $coupon->id, '_coupon_revenue', $discounts );

		// Track the total discount this coupon has given
		$total_discount = absint( get_post_meta( $coupon->id, '_total_discount', true ) );
		$total_discount += $discount;
		update_post_meta( $coupon->id, '_total_discount', $total_discount );

		// Track total revenue generated by this coupon
		$total_revenue = absint( get_post_meta( $coupon->id, '_total_revenue', true ) );
		$total_revenue =+ $order_total;
		update_post_meta( $coupon->id, '_total_revenue', $total_revenue );
	...
	private function save_coupon_note( $coupon, $discount, $order_id, $order_total, $user, $time ) {
		...
		$note = '<a href="' . esc_url( $order_url ) . '"><b>' . __( 'Order', 'wc_coupon_campaigns' ) . ' #' . $order_id . '</b></a><br/>
				 ' . __( 'Customer:', 'wc_coupon_campaigns' ) . ' <a href="' . admin_url( 'user-edit.php?user_id=' . $user->ID ) . '">' . $user->data->user_login . '</a><br/>
				 ' . __( 'Discount:', 'wc_coupon_campaigns' ) . ' ' . woocommerce_price( $discount ) . '<br/>
				 ' . __( 'Revenue:', 'wc_coupon_campaigns' ) . ' ' . woocommerce_price( $order_total );
	...
	public function display_order_campaigns( $order ) {
		...
		$campaigns = wp_get_post_terms( $order->id, $this->tax );
	...
	public function coupon_info() {
		...
		if( isset( $_GET['coupon'] ) ) {
			...
			switch( $coupon->discount_type ) {
				case 'fixed_cart': $amount = woocommerce_price( $coupon->amount ); break;
				case 'percent': $amount = $coupon->amount . '%'; break;
				case 'fixed_product': $amount = woocommerce_price( $coupon->amount ); break;
				case 'percent_product': $amount = $coupon->amount . '%'; break;
				case 'sign_up_fee': $amount = woocommerce_price( $coupon->amount ); break;
				case 'recurring_fee': $amount = woocommerce_price( $coupon->amount ); break;
