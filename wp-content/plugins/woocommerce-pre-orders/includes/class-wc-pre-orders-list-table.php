		...
		switch ( $column_name ) {
			...
			case 'status':
				...
				if ( WC_Pre_Orders_Manager::can_pre_order_be_changed_to( 'cancelled', $order ) ) {
					...
					$actions['cancel'] = sprintf( '<a href="%s">%s</a>', add_query_arg( array( 'action' => 'cancel', 'product_id' => $product_id ), wp_nonce_url( $action_url, 'cancel_pre_order', 'cancel_pre_order_nonce' ) ), __( 'Cancel', 'wc-pre-orders' ) );
				}
