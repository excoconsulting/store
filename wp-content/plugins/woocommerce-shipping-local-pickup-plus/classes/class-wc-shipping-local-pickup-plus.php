...
class WC_Shipping_Local_Pickup_Plus extends WC_Shipping_Method {
	...
	public function __construct() {
		...
		if ( $this->is_available( array() ) ) {
			...
			// add local pickup localtion selection row to recurring shipping rates tables in Subscriptions v2.0.18+
			add_action( 'woocommerce_subscriptions_after_recurring_shipping_rates', array( $this, 'render_pickup_location_selection_row' ) );
	 ...
	 * doesn't actually define this method, it's assumed to exist by the client
	 * code
	 *
	...
	public function process_admin_options() {
		...
		for ( $i = 0, $ix = count( $ids ); $i < $ix; $i++ ) {
			...
			$pickup_location = array(
				...
				'cost'      => isset( $costs[ $i ] ) && '' !== $costs[ $i ] ? abs( $costs[ $i ] )    : null,
				'id'        => $id,
	...
	public function update_shipping_packages( $packages ) {
		...
		// WC allows for multiple shipping packages
		foreach ( $packages as $package_index => $package ) {
			...
			if ( wc_local_pickup_plus()->is_subscriptions_active() && isset( $package['recurring_cart_key'] ) && 'none' !== $package['recurring_cart_key'] ) {
			...
			if ( count( $other_products ) == 0 && count( $found_products ) > 0 ) {
				...
				$new_packages[ $package_index ] = $package;
				...
				$new_packages[ $package_index ] = $found_products_package;
				$new_packages[ $package_index ] = $other_products_package;
	...
	public function render_pickup_location_selection_row( $package_index ) {
		...
		//  while we do check this above in review_order_shipping_pickup_location(), this check ensures we don't display the select on the Cart page for the Subscriptions Recurring Totals section
		if ( ! is_checkout() ) {
			return;
		}
