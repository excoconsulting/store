...
class WC_Deposits_Plans_Admin {
	...
	public function maybe_save_plan() {
		...
		if ( isset( $_POST['plan_name'] ) ) {
			...
			$existing_schedule = $wpdb->get_results( $wpdb->prepare( "SELECT schedule_index, plan_id, amount, interval_amount, interval_unit FROM {$wpdb->wc_deposits_payment_plans_schedule} WHERE plan_id = %d;", $plan_id ) );

			if ( $existing_schedule !== $payment_schedule ) {
				// Clear and update
