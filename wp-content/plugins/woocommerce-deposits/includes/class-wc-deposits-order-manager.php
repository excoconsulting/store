...
class WC_Deposits_Order_Manager {
	...
	public function woocommerce_get_order_item_totals( $total_rows, $order ) {
		if ( $this->has_deposit( $order ) ) {
			...
			// rebuild the order total rows to include
			// our own information.
			foreach ( $total_rows as $row => $value ) {
				if ( 'order_total' === $row ) {
					$new_total_rows['total_payable'] = array(
						'label' => __( 'Payable in Total', 'woocommerce-deposits' ),
						'value' => wc_price( $total_payable ) . $tax_element
					);
	...
	/**
	 * Adds pending-deposit order status for inclusion for
	 * payment processes.
	 *
	 * @since 1.1.8
	 * @version 1.1.8
