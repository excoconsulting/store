...
if ( ! class_exists( 'WC_Shipping_Distance_Rate' ) ) {
	...
	class WC_Shipping_Distance_Rate extends WC_Shipping_Method {
		...
		public function __construct() {
			...
			$this->method_title = __( 'Distance Rate', 'wc-distance-rate' );
		...
		public function init_form_fields() {
			$this->form_fields  = array(
				'enabled'          => array(
					'title'           => __( 'Enable/Disable', 'wc-distance-rate' ),
					'type'            => 'checkbox',
					'label'           => __( 'Enable this shipping method', 'wc-distance-rate' ),
					'default'         => 'no'
				),
				'title'            => array(
					'title'           => __( 'Method Title', 'wc-distance-rate' ),
					'type'            => 'text',
					'description'     => __( 'This controls the title which the user sees during checkout.', 'wc-distance-rate' ),
					'default'         => __( 'Distance Rate', 'wc-distance-rate' )
				),
				'availability'  => array(
					'title'           => __( 'Method Availability', 'wc-distance-rate' ),
					'type'            => 'select',
					'default'         => 'all',
					'class'           => 'availability',
					'options'         => array(
						'all'            => __( 'All Countries', 'wc-distance-rate' ),
						'specific'       => __( 'Specific Countries', 'wc-distance-rate' ),
					),
				),
				'countries'        => array(
					'title'           => __( 'Specific Countries', 'wc-distance-rate' ),
					'type'            => 'multiselect',
					'class'           => 'chosen_select',
					'css'             => 'width: 450px;',
					'default'         => '',
					...
					'custom_attributes' => array(
						'data-placeholder' => __( 'Select some countries', 'wc-distance-rate' )
					)
				),
				'tax_status' => array(
					'title' 		=> __( 'Tax Status', 'wc-distance-rate' ),
					'type' 			=> 'select',
					'default' 		=> 'taxable',
					'options'		=> array(
						'taxable' 	=> __( 'Taxable', 'wc-distance-rate' ),
						'none' 		=> _x( 'None', 'Tax status', 'wc-distance-rate' )
					),
				),
				'api_key'		=> array(
					'title' => __( 'API Key', 'wc-distance-rate' ),
					'type'	=> 'text',
					'description'	=> __( 'Your <a href="http://docs.woothemes.com/document/woocommerce-distance-rate-shipping/#section-3">Google API Key</a>', 'wc-distance-rate' ),
				),
				'mode'  => array(
					'title'           => __( 'Transportation Mode', 'wc-distance-rate' ),
					'type'            => 'select',
					'default'         => 'driving',
					'options'         => array(
						'driving'     => __( 'Driving', 'wc-distance-rate' ),
						'walking'     => __( 'Walking', 'wc-distance-rate' ),
						'bicycling'   => __( 'Bicycling', 'wc-distance-rate' ),
					),
				),
				'avoid'  => array(
					'title'           => __( 'Avoid', 'wc-distance-rate' ),
					'type'            => 'select',
					'default'         => 'none',
					'options'         => array(
						'none'      => __( 'None', 'wc-distance-rate' ),
						'tolls'     => __( 'Tolls', 'wc-distance-rate' ),
						'highways'  => __( 'Highways', 'wc-distance-rate' ),
						'ferries'   => __( 'Ferries', 'wc-distance-rate' ),
					),
				),
				'unit'  => array(
					'title'           => __( 'Distance Unit', 'wc-distance-rate' ),
					'type'            => 'select',
					'default'         => 'metric',
					'options'         => array(
						'metric'      => __( 'Metric', 'wc-distance-rate' ),
						'imperial'    => __( 'Imperial', 'wc-distance-rate' ),
					),
				),
				'show_distance' => array(
					'title'           => __( 'Show distance', 'wc-distance-rate' ),
					'type'            => 'checkbox',
					'label'           => __( 'Show the distance next to the shipping cost for the customer.', 'wc-distance-rate' ),
					'default'         => 'yes'
				),
				'debug' => array(
					'title'			=> __( 'Debug Mode', 'wc-distance-rate' ),
					'type'			=> 'checkbox',
					'label'			=> __( 'Enable/Disable Debug Mode, display API calls on frontend', 'wc-distance-rate' ),
					'default'		=> 'no',

				),
				'distance_rate_address' => array(
					'title' 		=> __( 'Shipping Address', 'wc-distance-rate' ),
					'type' 			=> 'title',
					'description' 	=> __( 'Please enter the address that you are shipping from below to work out the distance of the customer from the shipping location.', 'wc-distance-rate' ),
				),
				'address_1' => array(
					'title'           => __( 'Address 1', 'wc-distance-rate' ),
					'type'            => 'text',
					'description'     => __( 'First address line of where you are shipping from.', 'wc-distance-rate' ),
				),
				'address_2' => array(
					'title'           => __( 'Address 2', 'wc-distance-rate' ),
					'type'            => 'text',
					'description'     => __( 'Second address line of where you are shipping from.', 'wc-distance-rate' ),
				),
				'city' => array(
					'title'           => __( 'City', 'wc-distance-rate' ),
					'type'            => 'text',
					'description'     => __( 'City of where you are shipping from.', 'wc-distance-rate' ),
				),
				'postal_code'             => array(
					'title'           => __( 'Zip/Postal Code', 'wc-distance-rate' ),
					'type'            => 'text',
					'description'     => __( 'Zip or Postal Code of where you are shipping from.', 'wc-distance-rate' ),
				),
				'state_province' => array(
					'title'           => __( 'State/Province', 'wc-distance-rate' ),
					'type'            => 'text',
					'description'     => __( 'State/Province of where you are shipping from.', 'wc-distance-rate' ),
				),
				'country' => array(
					'title'           => __( 'Country', 'wc-distance-rate' ),
					'type'            => 'text',
					'description'     => __( 'Country of where you are shipping from.', 'wc-distance-rate' ),
				),
		...
		public function generate_distance_rate_rates_table_html() {
			...
			?>
			<tr valign="top">
				<th scope="row" class="titledesc"><?php _e( 'Distance Rate Rules', 'wc-distance-rate' ); ?>:</th>
				<td class="forminp" id="<?php echo $this->id; ?>_rules">
					<table class="shippingrows widefat" cellspacing="0">
						<thead>
							<tr>
								<th class="check-column"><input type="checkbox"></th>
								<th><?php _e( 'Condition', 'wc-distance-rate' ); ?> <a class="tips" data-tip="<?php _e( 'On what condition must the rule be applied.', 'wc-distance-rate' ); ?>">[?]</a></th>
								<th><?php _e( 'Min', 'wc-distance-rate' ); ?> <a class="tips" data-tip="<?php _e( 'Minimum condition value, leave blank for no limit. Travel time based in minutes.', 'wc-distance-rate' ); ?>">[?]</a></th>
								<th><?php _e( 'Max', 'wc-distance-rate' ); ?> <a class="tips" data-tip="<?php _e( 'Maximum condition value, leave blank for no limit. Travel time based in minutes.', 'wc-distance-rate' ); ?>">[?]</a></th>
								<th><?php _e( 'Fixed Cost', 'wc-distance-rate' ); ?> <a class="tips" data-tip="<?php _e( 'Fixed cost for rule, exluding tax.', 'wc-distance-rate' ); ?>">[?]</a></th>
								<th style="padding-right: 20px;"><?php _e( 'Cost Per Distance / Minute', 'wc-distance-rate' ); ?> <a class="tips" data-tip="<?php _e( 'Cost per distance unit, or cost per minute for total travel time, excluding tax.', 'wc-distance-rate' ); ?>">[?]</a></th>
								<th><?php _e( 'Handling Fee', 'wc-distance-rate' ); ?> <a class="tips" data-tip="<?php _e( 'Fee excluding tax. Enter an amount, e.g. 2.50, or a percentage, e.g. 5%.', 'wc-distance-rate' ); ?>">[?]</a></th>
								<th><?php _e( 'Break', 'wc-distance-rate' ); ?> <a class="tips" data-tip="<?php _e( 'Check to not continue processing rules below the selected rule.', 'wc-distance-rate' ); ?>">[?]</a></th>
							</tr>
						</thead>
						<tfoot>
							<tr>
								<th colspan="8"><a href="#" class="add button"><?php _e( 'Add Distance Rule', 'wc-distance-rate' ); ?></a> <a href="#" class="remove button"><?php _e( 'Delete selected rule', 'wc-distance-rate' ); ?></a></th>
							</tr>
						</tfoot>
								...
								foreach ( $this->rules as $rule ) {
									...
									echo '<tr class="distance_rule">
										...
										<td><select name="' .  $this->id . '_condition[' . $i . ']">
											<option value="distance" ' . selected( $rule['condition'], 'distance', false ) . '>' . __( 'Distance', 'wc-distance-rate' ) . '</option>
											<option value="time" ' . selected( $rule['condition'], 'time', false ) . '>' . __( 'Total Travel Time', 'wc-distance-rate' ) . '</option>
											<option value="weight" ' . selected( $rule['condition'], 'weight', false ) . '>' . __( 'Weight', 'wc-distance-rate' ) . '</option>
											<option value="total" ' . selected( $rule['condition'], 'total', false ) . '>' . __( 'Order Total', 'wc-distance-rate' ) . '</option>
											<option value="quantity" ' . selected( $rule['condition'], 'quantity', false ) . '>' . __( 'Quantity', 'wc-distance-rate' ) . '</option>
										</select></td>
					...
					jQuery(function() {
						...
						jQuery('#<?php echo $this->id; ?>_rules').on( 'click', 'a.add', function(){
							...
							jQuery('<tr class="distance_rule">\
								...
								<td><select name="<?php echo $this->id; ?>_condition[' + size + ']">\
									<option value="distance"><?php _e( "Distance", "wc-distance-rate" ); ?></option>\
									<option value="time"><?php _e( "Total Travel Time", "wc-distance-rate" ); ?></option>\
									<option value="weight"><?php _e( "Weight", "wc-distance-rate" ); ?></option>\
									<option value="total"><?php _e( "Order Total", "wc-distance-rate" ); ?></option>\
									<option value="quantity"><?php _e( "Quantity", "wc-distance-rate" ); ?></option>\
								</select></td>\
						...
						jQuery('#<?php echo $this->id; ?>_rules').on( 'click', 'a.remove', function(){
							var answer = confirm("<?php _e( 'Delete the selected rule?', 'wc-distance-rate' ); ?>");
							if (answer) {
		...
		public function process_rules() {
			...
			if ( isset( $_POST[ $this->id . '_condition'] ) ) $rule_condition  = array_map( 'stripslashes', $_POST[ $this->id . '_condition'] );
			if ( isset( $_POST[ $this->id . '_min'] ) ) $rule_min  = array_map( 'stripslashes', $_POST[ $this->id . '_min'] );
			if ( isset( $_POST[ $this->id . '_max'] ) ) $rule_max  = array_map( 'stripslashes', $_POST[ $this->id . '_max'] );
			if ( isset( $_POST[ $this->id . '_cost'] ) ) $rule_cost  = array_map( 'stripslashes', $_POST[ $this->id . '_cost'] );
			if ( isset( $_POST[ $this->id . '_cost_unit'] ) ) $rule_cost_unit  = array_map( 'stripslashes', $_POST[ $this->id . '_cost_unit'] );
			if ( isset( $_POST[ $this->id . '_fee'] ) ) $rule_fee = array_map( 'stripslashes', $_POST[ $this->id . '_fee'] );
			if ( isset( $_POST[ $this->id . '_break'] ) ) $rule_break = array_map( 'stripslashes', $_POST[ $this->id . '_break'] );
				...
				switch ( $rule['condition'] ) {
					case 'distance':
						...
						if ( ! is_null( $rule_cost ) ) {
							...
							$this->show_notice( __( 'Sorry, that shipping location is beyond our shipping radius.', 'wc-distance-rate' ) );
						}
					break;
					...
					case 'time':
						...
						if ( ! is_null( $rule_cost ) ) {
							...
							$this->show_notice( __( 'Sorry, that shipping location is beyond our shipping travel time.', 'wc-distance-rate' ) );
						}
					break;
					...
					case 'weight':
						...
						if ( ! is_null( $rule_cost ) ) {
							...
							$this->show_notice( __( 'Sorry, the total weight of your chosen items is beyond what we can ship.', 'wc-distance-rate' ) );
						}
					break;
					...
					case 'total':
						...
						if ( ! is_null( $rule_cost ) ) {
							...
							$this->show_notice( __( 'Sorry, the total order cost is beyond what we can ship.', 'wc-distance-rate' ) );
						}
					break;
					...
					case 'quantity':
						...
						if ( ! is_null( $rule_cost ) ) {
							...
							$this->show_notice( __( 'Sorry, the total quantity of items is beyond what we can ship.', 'wc-distance-rate' ) );
						}
					break;
				}
