	...
	window.POS_BOOKING = {
		...
		init_calculation : function(){
			...
			$('.product-fields-table')
				.on('change', 'input, select', function() {
					...
					$('#wc-bookings-booking-cost').block({message: null, overlayCSS: {background: '#fff', backgroundSize: '16px 16px', opacity: 0.6}}).show();
					// Get blocks via ajax
		...
		create_posted_data : function(){
			...
			if( $('.booking_date_year').val() != '' && $('.booking_date_month').val() != '' && $('.booking_date_day').val() != '' ){
				...
				data['date']   = jQuery.datepicker.formatDate(pos_booking_form_args.date_format, date);
			}
			...
			data['_cost'] = parseFloat(_cost.text());
