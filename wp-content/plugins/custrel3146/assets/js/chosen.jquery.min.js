...
(function () {
	...
	e = function () {
		...
		e.prototype.add_group = function (e) {
			...
			this.parsed.push({array_index: t, group: !0, label: e.label, children: 0, disabled: e.disabled});
			s = e.childNodes;
			o = [];
...
(function () {
	...
	e = function () {
		...
		e.prototype.set_default_values = function () {
			...
			return this.inherit_select_classes = this.options.inherit_select_classes || !1
		};
...
(function () {
	...
	e.fn.extend({chosen: function (n) {
		...
		return r.name === "msie" && (r.version === "6.0" || r.version === "7.0" && document.documentMode === 7) ? this : this.each(function (r) {
			var i;
			i = e(this);
	...
	t = function (t) {
		...
		i.prototype.register_observers = function () {
			...
			return this.is_multiple ? this.search_choices.click(function (t) {
		...
		i.prototype.container_mousedown = function (t) {
			var n;
			if (!this.is_disabled) {
				n = t != null ? e(t.target).hasClass("search-choice-close") : !1;
		...
		i.prototype.results_build = function () {
			...
			for (n = 0, i = s.length; n < i; n++) {
				t = s[n];
				if (t.group)e += this.result_add_group(t); else if (!t.empty) {
					e += this.result_add_option(t);
					if (t.selected && this.is_multiple)this.choice_build(t); else if (t.selected && !this.is_multiple) {
		...
		i.prototype.choice_build = function (t) {
			...
			this.choices += 1;
			t.disabled ? r = '<li class="search-choice search-choice-disabled" id="' + n + '"><span>' + t.html + "</span></li>" : r = '<li class="search-choice" id="' + n + '"><span>' + t.html + '</span><a href="javascript:void(0)" class="search-choice-close" rel="' + t.array_index + '"></a></li>';
		...
		i.prototype.result_select = function (e) {
			...
			if (this.result_highlight) {
				...
				n = t.attr("id");
				this.result_clear_highlight();
				if (this.is_multiple)this.result_deactivate(t); else {
				...
				if (this.is_multiple)this.choice_build(r); else {
					this.selected_item.find("span").first().text(r.text);
					this.allow_single_deselect && this.single_deselect_control_build()
				}
				(!e.metaKey && !e.ctrlKey || !this.is_multiple) && this.results_hide();
				this.search_field.val("");
				(this.is_multiple || this.form_field_jq.val() !== this.current_value) && this.form_field_jq.trigger("change", {selected: this.form_field.options[r.options_index].value});
				this.current_value = this.form_field_jq.val();
				return this.search_field_scale()
			}
		};
			...
			for (d = 0, m = y.length; d < m; d++) {
				...
				if (!n.disabled && !n.empty)if (n.group)e("#" + n.dom_id).css("display", "none"); else if (!this.is_multiple || !n.selected) {
					...
					if (s.test(n.html)) {
						...
						if (i.length)for (v = 0, g = i.length; v < g; v++) {
							r = i[v];
							if (s.test(r)) {
								t = !0;
								f += 1
					...
					if (t) {
						...
						n.group_array_index != null && e("#" + this.results_data[n.group_array_index].dom_id).css("display", "list-item")
					} else {
						this.result_highlight && a === this.result_highlight.attr("id") && this.result_clear_highlight();
