...
;(function( $ ) {
	...
	$.WC_OD_Calendar.prototype = {
		...
		_init : function() {
			...
			this.$modal.dialog({
				...
				create: function( event, ui ) {
					...
					// Add datepickers
					self.$modalForm.find( '.date-field' ).bootstrapDP({
						autoclose: true,
						format: 'yyyy-mm-dd',
						...
						if ( 'start' === field ) {
							$end.bootstrapDP( 'setStartDate', ( value ? new Date( value ) : false ) );
						} else if ( 'end' === field ) {
							$start.bootstrapDP( 'setEndDate', ( value ? new Date( value ) : false ) );
						}
					});
		...
		eventAction: function( action, eventData ) {
			...
			// Call the correct function dinamically
		...
		loadEventForm: function( eventObject ) {
			...
			$start.bootstrapDP( 'update', eventData.start );
			$end.bootstrapDP( 'update', eventData.end );
			this.$modalForm.find( 'input[name="title"]' ).val( eventData.title );

			// Avoid that start date can be greater than the end date.
			$end.bootstrapDP( 'setStartDate', ( $start.val() ? new Date( $start.val() ) : false ) );
			$start.bootstrapDP( 'setEndDate', ( $end.val() ? new Date( $end.val() ) : false ) );
		},
	...
	$.WC_OD_Delivery_Calendar.prototype = $.extend( {}, $.WC_OD_Calendar.prototype, {
		_bindEvents : function() {
			...
			this.$modal.on( 'dialogcreate', function( event, ui ) {
				var $country = self.$modalForm.find( 'select[name="country"]' ),
				    $states = self.$modalForm.find( 'input[name="states"]' );
		...
		loadEventForm: function( eventObject ) {
			var eventData = this.getEventData( eventObject ),
			    $country = this.$modalForm.find( 'select[name="country"]' ),
			    $states = this.$modalForm.find( 'input[name="states"]' );

			$.WC_OD_Calendar.prototype.loadEventForm.apply( this, arguments );

			$country.select2( 'val', eventData.country );
			$states.val( eventData.states );
